...
{
    m_epfd = epoll_create(10); // 创建了epoll根节点
    if (m_epfd == -1)
    {
        perror("epoll创建失败!");
        exit(0);
...
int EpollDispatcher::dispatch(int timeout)
{
    ...
    for(int ii = 0; ii != count; ++ii){
        // 把有效循环，将对应的事件和文件描述符保存起来
        int events = m_events[ii].events;
        int fd = m_events[ii].data.fd;
        ...
        // 读事件
        if(events & EPOLLIN){
            m_eventLoop->eventActive(fd, static_cast<int>(FDEvent::ReadEvent));
        }

        // 写事件
        if(events & EPOLLOUT){
            m_eventLoop->eventActive(fd, static_cast<int>(FDEvent::WriteEvent));
        }
    }
...
int EpollDispatcher::epollCtl(int op)
{
    ...
    event.data.fd = m_channel->getSocket();// 要检查的文件描述符

    // 由于channel的读写事件由自己定义，需要判断进行设置
    //  按位与 只有对应的二进制11才为1，若不等于0，则等于ReadEvent对应的值
    
    int events = 0;
    if(m_channel->getEvent() & static_cast<int>(FDEvent::ReadEvent)){   //读事件
        // 保存相应的读事件
    ...
    int ret = epoll_ctl(m_epfd, op, m_channel->getSocket(), &event);
    return ret;
}
