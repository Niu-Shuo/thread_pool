 ...
 * @Version: 
 * @Author: Niu Shuo
 * @Date: 2024-06-13 19:51:23
 * @LastEditors: Niu Shuo
 * @LastEditTime: 2024-06-13 20:14:43
 */
#include <vector>
#include <string.h>
#include <arpa/inet.h>
...
namespace netrpc {
    ...
    void TinyPBCoder::encode(std::vector<AbstractProtocol::AbstractProtocolPtr> &messages, TcpBuffer::TcpBufferPtr out_buffer)
    {
    ...
    void TinyPBCoder::decode(std::vector<AbstractProtocol::AbstractProtocolPtr> &out_messages, TcpBuffer::TcpBufferPtr buffer)
    {
        while (1) {
            ...
            int ii;
            for (ii = start_index; ii < buffer->writeIndex(); ++ii) {
                if (tmp[ii] == TinyPBProtocol::PB_START) {
                    // 读下去四个字节。由于是网络字节序，需要转为主机字节序
                    if (ii + 1 < buffer->writeIndex()) {
                        pk_len = getInt32FromNetByte(&tmp[ii + 1]);
                        DEBUGLOG("get pk_len = %d", pk_len);
                    }

                    // 结束符的索引
                    int jj = ii + pk_len - 1;
                    if (jj >= buffer->writeIndex()) {
                        continue;
                    }
                    if (tmp[jj] == TinyPBProtocol::PB_END) {
                        start_index = ii;
                        end_index = jj;
                        parse_success = true;
                        break;
