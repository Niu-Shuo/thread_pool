 ...
 * @Version: 
 * @Author: Niu Shuo
 * @Date: 2024-06-13 20:23:02
 * @LastEditors: Niu Shuo
 * @LastEditTime: 2024-06-13 20:33:55
 */
...
namespace netrpc {
    ...
    public:
        NetAddr::NetAddrPtr select(std::vector<NetAddr::NetAddrPtr>& addrs, const TinyPBProtocol &invocation) {
            ...
            if (m_index >= (int)addrs.size()) {
                m_index = 0;
            }
            return addrs[m_index++];
        }
    private:
        int m_index = 0;
        std::mutex m_mutex;
    ...
    public:
        NetAddr::NetAddrPtr select(std::vector<NetAddr::NetAddrPtr> &addrs , const TinyPBProtocol &invocation) {
            ...
            auto it = m_selectors.find(invocation.m_method_name); // 对每个服务创建一个选择器
            std::shared_ptr<ConsistentHashSelector> selector;
            // check for updates
            // 检查invokers列表是否扩容或者缩容，如果不存在对应选择器，或者出现了扩缩容（会导致服务器列表的hash值不一致）则初始化选择器
            if (it == m_selectors.end() || it->second->identityHashCode != identityHashCode) {
                m_selectors.insert({invocation.m_method_name, std::make_shared<ConsistentHashSelector>(addrs, 160, identityHashCode)});
                selector = m_selectors[invocation.m_method_name];
            }
            else {
                selector = it->second;
            }
    ...
    private:
        ...
        std::unordered_map<std::string, std::shared_ptr<ConsistentHashSelector>> m_selectors;
        std::mutex m_mutex;
    };
