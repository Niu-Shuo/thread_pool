 ...
 * @Version: 
 * @Author: Niu Shuo
 * @Date: 2024-06-13 22:38:15
 * @LastEditors: Niu Shuo
 * @LastEditTime: 2024-06-14 10:48:42
 */
...
namespace netrpc {
    ...
    {
        auto req_protocol = std::make_shared<netrpc::TinyPBProtocol>();
        TinyPBProtocol tmp;
        ...
        auto addr = m_loadBalancer->select(m_addrs, tmp);
        m_client = std::make_shared<TcpClient>(m_peer_addr);

        auto channel = shared_from_this();

        auto timer_event = std::make_shared<TimerEvent>(my_controller->GetTimeout(), false, [my_controller, channel]() mutable {
        ...
        for (int ii = 0; ii < maxRetry; ++ii) {
            ...
            m_client->connect([req_protocol, this]() {
                ...
                getTcpClient()->writeMessage(req_protocol, [req_protocol, this, my_controller](AbstractProtocol::AbstractProtocolPtr) mutable {
                    ...
                    auto rsp_protocol = std::dynamic_pointer_cast<netrpc::TinyPBProtocol>(msg);
