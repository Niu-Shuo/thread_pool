/*
 * @Description: client测试
 * @Version: 
 * @Author: Niu Shuo
 * @Date: 2024-06-14 15:03:18
 * @LastEditors: Niu Shuo
 * @LastEditTime: 2024-06-14 15:15:11
 */
#include <assert.h>
...
void test_tcp_client() {
    ...
    auto addr = std::make_shared<netrpc::IPNetAddr>("127.0.0.1", 12345);
    netrpc::TcpClient client(addr);
    client.connect([addr, &client]() {
        DEBUGLOG("connect to [%s] success", addr->toString().c_str());
        auto message = std::make_shared<netrpc::TinyPBProtocol>();
        ...
        client.readMessage("123456789", [](netrpc::AbstractProtocol::AbstractProtocolPtr msg_ptr) {
            auto message = std::dynamic_pointer_cast<netrpc::TinyPBProtocol>(msg_ptr);
